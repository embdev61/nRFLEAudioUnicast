...
int bt_mgmt_init(void)
{
	...
	if (IS_ENABLED(CONFIG_SETTINGS)) {
		...
		if (ret) {
			LOG_ERR("bonding_clear_check Fail");
			return ret;
		}
		...
		if (IS_ENABLED(CONFIG_TESTING_BLE_ADDRESS_RANDOM)) {
			ret = bt_mgmt_bonding_clear();
			if (ret) {
				LOG_ERR("bt_mgmt_bonding_clear Fail");
				return ret;
			}
		}
	}
	...
	if (ret) {
		LOG_ERR("bt_mgmt_bonding_clear button_pressed Fail");
		return ret;
	}
	...
	if (pressed) {
		...
		if (ret) {
			LOG_ERR("bt_mgmt_ctlr_cfg_init Fail");
			return ret;
		}
	...
	if (ret) {
		LOG_ERR("local_identity_addr_print Fail");
		return ret;
	}
